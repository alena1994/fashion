 ...
 * Import classes
 */
use PDO, PDOStatement, SimpleXMLElement, Closure;
...
class Query
{
	...
	/**
	 * Выполнение SQL запроса и получение результирующего набора в виде XML данных
	 *
	 * @param   int     $options
	 *
	 * @access  public
	...
	{
		$xml = new SimpleXMLElement('<rows/>', $options);
		
		foreach ($this->toArray() as $rows)
		{
	...
	public function where($column, $compare, $value)
	{
		...
		$this->storage['where']['conditions'][] = [$this->prepareColumn($column), $this->prepareComparison($compare), $this->prepareValue($value), $this->storage['logical']];
	...
	public function having($column, $compare, $value)
	{
		...
		$this->storage['having']['conditions'][] = [$this->prepareColumn($column), $this->prepareComparison($compare), $this->prepareValue($value), $this->storage['logical']];
	...
	public function on($column1, $compare, $column2)
	{
		if (strcmp($this->operator, 'join') === 0)
		{
			...
			$this->storage['join'][$key]['conditions'][] = [$this->prepareColumn($column1), $this->prepareComparison($compare), $this->prepareColumn($column2), $this->storage['logical']];
	...
	public function getSql()
	{
		...
		if ($this->isSelect())
		{
			$parts = [$this->buildSelect(), $this->buildFrom(), $this->buildJoin(), $this->buildWhere(), $this->buildGroup(), $this->buildHaving(), $this->buildOrder(), $this->buildLimit(), $this->buildOffset()];
		}
		...
		if ($this->isUpdate())
		{
			$parts = [$this->buildUpdate(), $this->buildWhere(), $this->buildLimit(), $this->buildOffset()];
		}
		
		if ($this->isUpsert())
		{
			$parts = [$this->buildUpsert()];
		}
		
		if ($this->isDelete())
		{
			$parts = [$this->buildDelete(), $this->buildFrom(), $this->buildWhere(), $this->buildLimit(), $this->buildOffset()];
		}
	...
	protected function prepareValue($value)
	{
		...
		else if (is_string($value))
		{
			...
			$param = sprintf(':bindParam%d', $count);
