...
final class Fenric
{
	...
	public function registerResolveredSharedService($alias, callable $callable)
	{
		$this->registerSharedService($alias, function($resolver = null) use($callable)
		{
			...
			if (empty($responses[$resolver]))
			{
				$responses[$resolver] = call_user_func_array($callable, func_get_args());
			}
	 ...
	 */
	public function toLog($type, $message)
	{
		switch ($type)
		{
			case E_ERROR :
	...
	public function errorHandler($type, $message, $file, $line)
	{
		if ($type & error_reporting())
		{
			...
			$this->toLog($type, sprintf('%s (%s #%d)', $message, $file, $line));
		}
	}
	...
	public function exceptionHandler($exception)
	{
		...
		$this->toLog($severity, sprintf('%s (%s #%d)', $exception->getMessage(), $exception->getFile(), $exception->getLine()));
	...
	public function fatalityHandler()
	{
		if ($error = error_get_last())
		{
			...
			{
				$this->toLog($error['type'], sprintf('%s (%s #%d)', $error['message'], $error['file'], $error['line']));
	...
	public function crash($message, $file, $line)
	{
		...
		if ($this->is('not console'))
		{
			...
			{
				$this->callSharedService('request')->isAjax() ?
				
				$this->callSharedService('response')->setContent(['message' => $message, 'file' => $file, 'line' => $line], 'json') :
				
				$this->callSharedService('response')->setContent(new View('system/crash', ['message' => $message, 'file' => $file, 'line' => $line]), 'html');
			}
